...
class MainMenu: SKScene, SKPhysicsContactDelegate {
    ...
    func FormatMainMenuAction() {
        ...
        DUlabel.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: 200, height: 200))
        ...
        ELlabel.name = "EL"
        //define physics body and set initial physic parameters
        ELlabel.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: 200, height: 200))
        ...
        let MoveDown = SKAction.move(by: CGVector(dx:0,dy: -600), duration: 2)
        ...
        let Rotate = SKAction.rotate(byAngle: 1.571, duration: 1)
        let Wait = SKAction.wait(forDuration: 0.5)
        
        //define sequences for labels, including a call to repeat action x number of times
        let DUSequence = SKAction.repeat(SKAction.sequence([RightImpulse,LeftImpulse]), count: 5)
        let ELSequence = SKAction.repeat(SKAction.sequence([LeftImpulse,RightImpulse]), count: 5)
        let DUFinalMove = SKAction.move(to: CGPoint(x: 420, y: 1100), duration: 2.0)
        ...
        ELlabel.run(MoveDown,completion: {
            ...
            ELlabel.run(ELSequence, completion: {
                ...
                ELlabel.run(ELRotate, completion: {
    ...
    func RunOpeningSequence() {
        ...
        //on DU label, run initial sequence of sound and move down action, once complete run the code within
        DUlabel.run(SKAction.sequence([soundaction, MoveDown]),completion: {
            //once initial sequence is run, continue to run the defined DU sequence, once complete run the code within
            DUlabel.run(DUSequence, completion: {
                //pin DU label to negate physics and then set postition to the final one
                DUlabel.run(DUFinalMove)
